...
use crate::decode::sequences::{Sequence, SequenceDecodeTables, compute_offset};
...
pub fn execute_sequences(
    ...
    output.reserve(crate::frame::MAX_BLOCK_SIZE + WILDCOPY_OVERLENGTH);

    let out_base = output.as_mut_ptr();
    let mut op = unsafe { out_base.add(output.len()) };
    let op_limit = unsafe { out_base.add(output.len() + crate::frame::MAX_BLOCK_SIZE) };
    ...
    for seq in sequences {
        ...
        unsafe {
            if offset <= out_pos {
                crate::simd::scalar::copy_match(op, offset, ml);
            } else {
...
pub fn decode_execute_sequences(
    ...
    macro_rules! execute_seq {
        ($literal_length:expr, $match_length:expr, $offset:expr) => {{
            ...
            unsafe {
                if off <= out_pos {
                    crate::simd::scalar::copy_match(op, off, ml);
                } else {
...
unsafe fn copy_match_from_history(
    ...
    if remaining > 0 {
        unsafe {
            crate::simd::scalar::copy_match(op.add(from_history), offset, remaining);
        }
    }
}

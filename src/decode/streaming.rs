...
impl<R: Read> FrameDecoder<R> {
    /// Creates a decoder with [`DEFAULT_DECOMPRESS_LIMIT`](crate::DEFAULT_DECOMPRESS_LIMIT).
    pub fn new(reader: R) -> Self {
        Self::with_limit(reader, crate::DEFAULT_DECOMPRESS_LIMIT)
    }
    ...
    fn decode_compressed_block(&mut self, block_size: usize) -> io::Result<()> {
        ...
        let before = self.output_buf.len();
        crate::decode::exec::decode_execute_sequences(
            seq_data,
            num_sequences,
            &self.seq_tables,
            &mut self.rep_offsets,

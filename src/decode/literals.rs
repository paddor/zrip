...
use crate::huffman::decode::{
    decode_4_streams, decode_4_streams_into, decode_single_stream, decode_single_stream_vec,
};
use crate::huffman::weights::{
    build_huffman_decode_table, build_huffman_decode_table_into, parse_huffman_weights,
};
...
pub struct LiteralsSection {
    pub data: Vec<u8>,
    pub huffman_table: Option<(Vec<HuffmanDecodeEntry>, u8)>,
}

pub fn decode_literals(
    data: &[u8],
    prev_huffman: Option<&(Vec<HuffmanDecodeEntry>, u8)>,
) -> Result<(LiteralsSection, usize), DecompressError> {
    if data.is_empty() {
        return Err(DecompressError::CorruptLiterals);
    }

    let header_byte = data[0];
    let block_type = match header_byte & 0x03 {
    ...
    match block_type {
        LiteralsBlockType::Raw => decode_raw_literals(data),
        LiteralsBlockType::Rle => decode_rle_literals(data),
        LiteralsBlockType::Compressed => decode_compressed_literals(data, false, None),
        LiteralsBlockType::Treeless => decode_compressed_literals(data, true, prev_huffman),
    }
}

fn decode_raw_literals(data: &[u8]) -> Result<(LiteralsSection, usize), DecompressError> {
    let header_byte = data[0];
    ...
    let (regen_size, header_size) = match size_format {
        ...
        1 => {
            ...
            let size = ((header_byte >> 4) as usize) | ((data[1] as usize) << 4);
            (size, 2)
        }
        3 => {
            if data.len() < 3 {
            ...
            let size = ((header_byte >> 4) as usize)
                | ((data[1] as usize) << 4)
                | ((data[2] as usize) << 12);
            (size, 3)
        }
...
fn decode_rle_literals(data: &[u8]) -> Result<(LiteralsSection, usize), DecompressError> {
    let header_byte = data[0];
    let size_format = (header_byte >> 2) & 0x03;
...
fn decode_compressed_literals(
    ...
    let (regen_size, compressed_size, num_streams, header_size) = match size_format {
        0 => {
            ...
            let b0 = header_byte as usize;
            let b1 = data[1] as usize;
            let b2 = data[2] as usize;
            let both_sizes = (b0 >> 4) | (b1 << 4) | (b2 << 12);
            let regen = both_sizes & 0x3FF;
            let comp = both_sizes >> 10;
            (regen, comp, 1usize, 3)
        }
        1 => {
            if data.len() < 3 {
        ...
        2 => {
            ...
            let both_sizes = (b0 >> 4) | (b1 << 4) | (b2 << 12) | (b3 << 20);
            let regen = both_sizes & 0x3FFF;
            let comp = both_sizes >> 14;
            (regen, comp, 4usize, 4)
        }
        3 => {
            if data.len() < 5 {
            ...
            let both_sizes = (b0 >> 4) | (b1 << 4) | (b2 << 12) | (b3 << 20) | (b4 << 28);
            let regen = both_sizes & 0x3FFFF;
            let comp = both_sizes >> 18;
            (regen, comp, 4usize, 5)
        }
        _ => unreachable!(),
    };
    ...
    let (huffman_table, huf_consumed) = if treeless {
        let table = prev_huffman
            .ok_or(DecompressError::CorruptLiterals)?
            .clone();
        (table, 0)
    } else {
        let (weights, consumed) = parse_huffman_weights(stream_data)?;
        let table = build_huffman_decode_table(&weights)?;
        (table, consumed)
    };

    let compressed_stream = &stream_data[huf_consumed..];

    let literals_data = if num_streams == 1 {
...
pub(crate) fn decode_literals_ws(
    ...
    let consumed = match block_type {
        LiteralsBlockType::Raw => {
            ...
            if regen_size > crate::frame::MAX_BLOCK_SIZE {
                return Err(DecompressError::CorruptLiterals);
            }
            if data.len() < header_size + regen_size {
                return Err(DecompressError::CorruptLiterals);

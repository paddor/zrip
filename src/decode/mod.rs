...
use crate::block::{BlockType, parse_block_header};
use crate::decode::exec::decode_execute_sequences;
use crate::decode::literals::decode_literals_ws;
use crate::decode::sequences::{
    SequenceDecodeTables, parse_sequence_count, parse_sequence_tables_ws,
};
use crate::error::DecompressError;
use crate::frame::MAX_WINDOW_SIZE;
use crate::frame::header::parse_frame_header;
use crate::huffman::HuffmanDecodeEntry;
#[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
use crate::simd::CpuTier;
use crate::xxhash::Xxh64State;
...
pub(crate) struct BlockDecodeWorkspace {
    ...
    pub fse_build_buf: Vec<crate::fse::FseDecodeEntry>,
}
...
pub fn decompress_into(input: &[u8], output: &mut Vec<u8>) -> Result<usize, DecompressError> {
    let max_output = crate::DEFAULT_DECOMPRESS_LIMIT;
...
pub(crate) fn decompress_frame(
    ...
    let mut seq_tables = if let Some(d) = dict {
        ...
        if let Some((t, l)) = d.of_table() {
            st.of_table = crate::fse::promote_of_table(t);
            st.of_accuracy = l;
        }
        if let Some((t, l)) = d.ml_table() {
            st.ml_table = crate::fse::promote_ml_table(t);
            st.ml_accuracy = l;
        }
        if let Some((t, l)) = d.ll_table() {
            st.ll_table = crate::fse::promote_ll_table(t);
            st.ll_accuracy = l;
        }
        st
    } else {
    ...
    loop {
        ...
        if block_size > crate::frame::MAX_BLOCK_SIZE {
...
fn decode_compressed_block(
    ...
    {
        if crate::simd::cpu_tier() >= CpuTier::Avx2 {
            ...
            if output.len() - before > crate::frame::MAX_BLOCK_SIZE {
                return Err(DecompressError::CorruptSequences);
            }
            return Ok(());
        }
    ...
    {
        if crate::simd::cpu_tier() >= CpuTier::Neon {
...
fn decode_execute_block_avx2(
    ...
    crate::simd::x86_64::decode::decode_execute_avx2_safe(
        seq_data,
        num_sequences,
        tables,
        rep_offsets,
...
fn decode_execute_block_neon(
    ...
    crate::simd::aarch64::decode::decode_execute_neon_safe(
        seq_data,
        num_sequences,
        tables,
        rep_offsets,

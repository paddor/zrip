...
Two panels: Compressible (left), Incompressible (right), each with its own
Y-axis scale. X-axis: encode throughput (MB/s, log scale). Y-axis: compression
ratio. Connected points per codec, each point a level.
...
def generate_svg(data):
    ...
    p_top = top_margin
    p_bot = top_margin + panel_h

    panel_defs = [
        (70, 410, "Compressible"),
        (440, 830, "Incompressible"),
    ]

    # Shared X-axis (log encode MB/s) across both panels
    ...
    panel_y_ranges = {}
    for _, _, group_name in panel_defs:
        ratios = [v[1] for k, v in points.items() if k[2] == group_name]
        if ratios:
            rmin = min(ratios)
            rmax = max(ratios)
            margin = (rmax - rmin) * 0.15 if rmax > rmin else rmax * 0.1
            panel_y_ranges[group_name] = (rmin - margin, rmax + margin)
        else:
            panel_y_ranges[group_name] = (0.8, 2.0)
    ...
    for xl, xr, group_name in panel_defs:
        pw = xr - xl
        ratio_min, ratio_max = panel_y_ranges[group_name]
        ...
        for codec in CODEC_ORDER:
            ...
            for cp in codec_points[1:]:
                prev = deduped[-1]
                if abs(cp[1] - prev[1]) / prev[1] > 0.02 or abs(cp[2] - prev[2]) / prev[2] > 0.02:
                    deduped.append(cp)
            if len(deduped) == 1 and len(codec_points) > 1:
                codec_points = deduped
            # else keep all for codecs with real variation
            ...
            # Decide which points get labels. For dense sequences (>5 points),
            # label only the endpoints and every 3rd intermediate point.
            label_indices = set()
            n = len(codec_points)
            if n <= 5:
            ...
            # Collect placed label bboxes for collision avoidance
            placed = []
            ...
            for idx, (level, enc, rat) in enumerate(codec_points):
                ...
                for cx, cy, anchor in candidates:
                    ...
                    collides = False
                    for pb in placed:
                        if bbox[0] < pb[2] and bbox[2] > pb[0] and bbox[1] < pb[3] and bbox[3] > pb[1]:
                            collides = True
                            break
                    if not collides:
                        best = (cx, cy, anchor)
    ...
    total_w = sum(len(label) * 6.2 + 24 for _, label in legend_items) + 12 * (len(legend_items) - 1)
    lx = mid_x - total_w / 2

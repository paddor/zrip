#!/usr/bin/env python3
"""Generate matrix SVG chart from cached zrip_bench JSONL data.

Rows: compressibility groups (high/medium/low).
Columns: compression levels (L-1, L1, L3).
Stacked bars: compress + transfer@100MB/s + decompress.

Reads ~/.cache/zrip/L{n}/{codec}.jsonl, writes matrix.svg.

Usage:
    python3 scripts/plot_matrix.py [output_dir]
"""
import json
import math
import os
...
def compute_stacks(data):
    """Aggregate pipeline times: compress + transfer@100MB/s + decompress."""
    groups = GROUPS
...
def generate_svg(data):
    ...
    # per-panel y-axis scale (panels = compressibility groups)
    panel_y_max = {}
    for group_name, _ in GROUPS:
        pmax = 0
        for level in LEVELS:
            for c in CODEC_ORDER:
                v = stacks.get((c, level, group_name))
                if v:
                    pmax = max(pmax, sum(v))
        panel_y_max[group_name] = pmax * 1.15
    ...
    L.append(
        ...
        f'zstd Pipeline @100 MB/s: Aggregate across corpus (lower is better)'
        f'</text>'
    )
    if hw_label:
        L.append(
    ...
    for pi, (group_name, _) in enumerate(GROUPS):
        p_top = panel_tops[pi]
        p_bot = p_top + panel_h
        ...
        p_y_max = panel_y_max[group_name]
        if p_y_max == 0:
            continue
        ...
        # bars: columns are levels
        n_cols = len(COLUMN_LEVELS)
        col_w = plot_w / n_cols
        max_codecs = max(
            len([c for c in CODEC_ORDER if stacks.get((c, lv, group_name))])
            for lv in COLUMN_LEVELS
        )
        if max_codecs == 0:
            continue
        bar_w = min(col_w * 0.7 / max_codecs, 50)
        inner_gap = bar_w * 0.15

        for li, level in enumerate(COLUMN_LEVELS):
            col_x = x_left + li * col_w + (col_w * 0.2) / 2
            codecs = [c for c in CODEC_ORDER
                      if stacks.get((c, level, group_name))]

            for ci, codec in enumerate(codecs):
                comp, transfer, decomp = stacks[(codec, level, group_name)]
                main_c, xfer_c = COLORS[codec]

                bx = col_x + ci * (bar_w + inner_gap / max_codecs)
...
def main():
    ...
    path = os.path.join(output_dir, "matrix.svg")
